// content-script.js
// Runs injected into the active browser tab when user clicks "Save This Page".
// Responsibilities:
//   1. Extract the main readable content from the page
//      - Primary: Mozilla Readability (loaded inline via fetch/eval to avoid CDN issues)
//      - Fallback: document.body.innerText
//   2. Send the extracted content back to background.js via chrome.runtime.sendMessage
//
// NOTE: This file is injected programmatically via chrome.scripting.executeScript.
//       It is NOT listed in manifest content_scripts (on-demand injection only).

(async function extractPageContent() {
  // Only run for an actual Save click — background.js sets this flag right
  // before injecting this file for that flow. Without this guard, the
  // highlight-to-explain command (below) would also re-inject this same file
  // just to register its listener, and this IIFE would silently auto-save a
  // bookmark the first time a user highlights text on a page they never
  // clicked "Save This Page" on.
  if (!window.__growCareerExtractMode) return;

  // Prevent double-injection: if this script already ran, skip
  if (window.__growCareerExtracted) return;
  window.__growCareerExtracted = true;
  ...
  try {
    ...
    // We dynamically import Readability from a CDN via fetch+eval approach,
    // or fall back gracefully if that's not possible in this context.
    text = await extractWithReadability();
  } catch (e) {
    console.warn('[GrowCareer] Readability failed, falling back to innerText:', e);
  }

  // Fallback: use innerText of body
...
async function extractWithReadability() {
  // Fetch Readability.js from Mozilla's public CDN via esm.sh / unpkg
  // We inline-load it so the extension bundle stays lightweight (no bundler needed)
  const readabilityUrl = 'https://unpkg.com/@mozilla/readability@0.5.0/Readability.js';
  ...
  try {
    ...
    // Execute it in a sandboxed function to extract the Readability constructor
    // eslint-disable-next-line no-new-func
    const moduleFactory = new Function('module', 'exports', scriptText);
    const mod = { exports: {} };
    moduleFactory(mod, mod.exports);
    ReadabilityClass = mod.exports.Readability || mod.exports;
  } catch (fetchErr) {
    throw new Error('Could not load Readability: ' + fetchErr.message);
  }
