...
var Te = (e, t, n = {}) => {
	...
	static request(e, t, n, r, i, a, o) {
		...
		if (x.fetch && x.AbortController) return this.fetchRequest(e, t, n, r, i, a, o);
...
var vt = class e {
	...
	subscribe(e, t = this.timeout) {
		if (this.socket.isConnected() || this.socket.connect(), this.channelAdapter.isClosed()) {
			let { config: { broadcast: n, presence: r, private: i, postgres_changes_options: a } } = this.params, o = this.bindings.postgres_changes?.map((e) => e.filter) ?? [], s = !!this.bindings[T.PRESENCE] && this.bindings[T.PRESENCE].length > 0 || this.params.config.presence?.enabled === !0, c = {}, l = Object.assign({
				broadcast: n,
			...
			this.socket.accessTokenValue && (c.access_token = this.socket.accessTokenValue), this._onError((t) => {
				e?.(E.CHANNEL_ERROR, st(t));
			}), this._onClose(() => e?.(E.CLOSED)), this.updateJoinPayload(Object.assign({ config: l }, c)), this._updateFilterMessage();
			...
			this.channelAdapter.subscribe(u).receive("ok", async ({ postgres_changes: t }) => {
				...
				e?.(E.CHANNEL_ERROR, Error(n, { cause: t }));
			}).receive("timeout", () => {
				e?.(E.TIMED_OUT);
			});
		}
	...
	_updatePostgresBindings(t, n) {
		...
		for (let o = 0; o < i; o++) {
			...
			if (d && d.event === s && e.isFilterValueEqual(d.schema, c) && e.isFilterValueEqual(d.table, l) && e.isFilterValueEqual(d.filter, u)) a.push(Object.assign(Object.assign({}, i), { id: d.id }));
				...
				this.unsubscribe(), this.state = y.errored, n?.(E.CHANNEL_ERROR, /* @__PURE__ */ Error("mismatch between server and client bindings for postgres changes"));
				return;
			}
		}
		this.bindings.postgres_changes = a, this.state != y.errored && n && n(E.SUBSCRIBED);
	}
	presenceState() {
	...
	on(e, t, n) {
		let r = this.channelAdapter.isJoined() || this.channelAdapter.isJoining(), i = e === T.PRESENCE || e === T.POSTGRES_CHANGES;
	...
	_on(t, n, r) {
		...
		if ((a instanceof gt || typeof a == "object" && a && typeof a.build == "function") && (n = Object.assign(Object.assign({}, n), { filter: a.build() })), i === T.POSTGRES_CHANGES && this.bindings[i]?.find((t) => e.isSamePostgresFilter(t.filter, n))) return this.socket.log("error", `duplicate \`postgres_changes\` binding for ${this.topic} ignored`, n), this;
...
var Tt = "\n  addEventListener(\"message\", (e) => {\n    if (e.data.event === \"start\") {\n      setInterval(() => postMessage({ event: \"keepAlive\" }), e.data.interval);\n    }\n  });", Et = class {
	...
	constructor(e, t) {
		...
		this.socketAdapter = new yt(e, n), this.httpEndpoint = Pe(e), this.fetch = this._resolveFetch(t?.fetch);
	}
	connect() {
	...
	channel(e, t = { config: {} }) {
		...
		if (r) return r;
		{
			let n = new vt(`realtime:${e}`, t, this);
	...
	_workerObjectUrl(e) {
		...
		if (e) t = e;
			...
			let e = new Blob([Tt], { type: "application/javascript" });
	...
	_initializeOptions(e) {
		...
		let t = {};
		t.timeout = e?.timeout ?? ye, t.heartbeatIntervalMs = e?.heartbeatIntervalMs ?? bt.HEARTBEAT_INTERVAL, this._disconnectOnEmptyChannelsAfterMs = e?.disconnectOnEmptyChannelsAfterMs ?? 2 * (e?.heartbeatIntervalMs ?? bt.HEARTBEAT_INTERVAL), t.transport = e?.transport ?? me.getWebSocketConstructor(), t.params = e?.params, t.logger = e?.logger, t.heartbeatCallback = this._wrapHeartbeatCallback(e?.heartbeatCallback), t.sessionStorage = e?.sessionStorage ?? wt(), t.reconnectAfterMs = e?.reconnectAfterMs ?? ((e) => xt[e - 1] || St);
		let n, r, i = e?.vsn ?? ve;
		switch (i) {
			case ge:
...
function At(e) {
	...
	return { async request({ method: n, path: r, query: i, body: a, headers: o }) {
		let s = Ot(e.baseUrl, r, i), c = await kt(e.auth), l = await t(s, {
			method: n,
			headers: {
		...
		if (!l.ok) {
			...
			throw new Dt(t?.message ?? `Request failed with status ${l.status}`, {
				status: l.status,
				icebergType: t?.type,
				icebergCode: t?.code,
				details: e
...
var Mt = class {
	...
	async listNamespaces(e) {
		let t = e ? { parent: jt(e.namespace) } : void 0;
	...
	async dropNamespace(e) {
		await this.client.request({
			method: "DELETE",
			path: `${this.prefix}/namespaces/${jt(e.namespace)}`
		});
	}
	...
	async namespaceExists(e) {
		try {
			...
			if (e instanceof Dt && e.status === 404) return !1;
			throw e;
		}
	}
	async createNamespaceIfNotExists(e, t) {
		try {
			...
			if (e instanceof Dt && e.status === 409) return;
			throw e;
		}
	}
};
...
var Nt = class {
	...
	async createTableIfNotExists(e, t) {
		try {
			...
			if (n instanceof Dt && n.status === 409) return await this.loadTable({
				namespace: e.namespace,
				name: t.name
			});
			throw n;
	...
	constructor(e) {
		...
		this.client = At({
			...
			auth: e.auth,
			fetchImpl: e.fetch
		}), this.accessDelegation = e.accessDelegation?.join(","), this.namespaceOps = new Mt(this.client, t), this.tableOps = new Nt(this.client, t, this.accessDelegation);
	}
	async listNamespaces(e) {
...
var Ht = class extends Bt {
	...
	toJSON() {
		return O(O({}, super.toJSON()), {}, { code: this.code });
	}
}, Ut = class extends Bt {
...
var Kt = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), qt = (e) => {
	...
	return t?.duplex && (i.duplex = t.duplex), O(O({}, i), n);
};
...
function tn(e = "storage") {
	return {
		...
		head: async (t, n, r, i) => en(t, "HEAD", n, O(O({}, r), {}, { noResolveJson: !0 }), i, void 0, e),
		remove: async (t, n, r, i, a) => en(t, "DELETE", n, i, a, r, e)
	};
}
var { get: nn, post: k, put: rn, head: an, remove: on } = tn("storage"), A = tn("vectors"), sn = class {
	...
	async uploadOrUpdate(e, t, n, r) {
		...
		return i.handleOperation(async () => {
			let a, o = O(O({}, pn), r), s = O(O({}, i.headers), e === "POST" && { "x-upsert": String(o.upsert) }), c = o.metadata;
			...
			let l = i._removeEmptyFolders(t), u = i._getFinalPath(l), d = await (e == "PUT" ? rn : k)(i.fetch, `${i.url}/object/${u}`, a, O({ headers: s }, o?.duplex ? { duplex: o.duplex } : {}));
			return {
				path: l,
				id: d.Id,
				fullPath: d.Key
	...
	async uploadToSignedUrl(e, t, n, r) {
		...
		return s.searchParams.set("token", t), i.handleOperation(async () => {
			let e, t = O(O({}, pn), r), o = O(O({}, i.headers), { "x-upsert": String(t.upsert) }), c = t.metadata;
			...
			return {
				path: a,
				fullPath: (await rn(i.fetch, s.toString(), e, O({ headers: o }, t?.duplex ? { duplex: t.duplex } : {}))).Key
			};
		});
	}
	async createSignedUploadUrl(e, t) {
		...
		return n.handleOperation(async () => {
			let r = n._getFinalPath(e), i = O({}, n.headers);
			t?.upsert && (i["x-upsert"] = "true");
			let a = await k(n.fetch, `${n.url}/object/upload/sign/${r}`, {}, { headers: i }), o = new URL(n.url + a.url), s = o.searchParams.get("token");
	...
	async copy(e, t, n) {
		var r = this;
		return r.handleOperation(async () => ({ path: (await k(r.fetch, `${r.url}/object/copy`, {
			bucketId: r.bucketId,
			sourceKey: e,
			destinationKey: t,
			destinationBucket: n?.destinationBucket,
	...
	async createSignedUrl(e, t, n) {
		...
		return r.handleOperation(async () => {
			let i = r._getFinalPath(e), a = typeof n?.transform == "object" && n.transform !== null && Object.keys(n.transform).length > 0, o = await k(r.fetch, `${r.url}/object/sign/${i}`, O(O({ expiresIn: t }, a ? { transform: n.transform } : {}), n?.versionId == null ? {} : { versionId: n.versionId }), { headers: r.headers }), s = new URLSearchParams();
	...
	async createSignedUrls(e, t, n) {
		...
		return r.handleOperation(async () => {
			...
			let o = a.toString();
			return i.map((e) => O(O({}, e), {}, { signedUrl: e.signedURL ? encodeURI(`${r.url}${e.signedURL}${o ? `&${o}` : ""}`) : null }));
		});
	}
	download(e, t, n) {
	...
	async info(e, t) {
		...
		let a = i.toString();
		return n.handleOperation(async () => Jt(await nn(n.fetch, `${n.url}/object/info/${r}${a ? `?${a}` : ""}`, { headers: n.headers })));
	}
	async exists(e) {
		var t = this;
	...
	async list(e, t, n) {
		var r = this;
		return r.handleOperation(async () => {
			let i = t?.sortBy ? O(O({}, fn.sortBy), t.sortBy) : fn.sortBy, a = O(O(O({}, fn), t), {}, {
				sortBy: i,
				prefix: e || ""
			});
			return await k(r.fetch, `${r.url}/object/list/${r.bucketId}`, a, { headers: r.headers }, n);
		});
	}
	async listV2(e, t) {
		var n = this;
		return n.handleOperation(async () => {
			let r = O({}, e);
			return await k(n.fetch, `${n.url}/object/list-v2/${n.bucketId}`, r, { headers: n.headers }, t);
		});
	}
	encodeMetadata(e) {
		return JSON.stringify(e);
...
}, hn = { "X-Client-Info": "storage-js/2.116.0" }, gn = class extends sn {
	constructor(e, t = {}, n, r) {
		let i = new URL(e);
		...
		let a = i.href.replace(/\/$/, ""), o = O(O({}, hn), t);
		super(a, o, n, "storage");
	}
	async listBuckets(e) {
		var t = this;
	...
	async emptyBucket(e) {
		var t = this;
		return t.handleOperation(async () => await k(t.fetch, `${t.url}/bucket/${e}/empty`, {}, { headers: t.headers }));
	}
	...
	async getBucketLifecycle(e) {
		var t = this;
		return t.handleOperation(async () => await nn(t.fetch, t.bucketLifecycleUrl(e), { headers: t.headers }));
	}
	async updateBucketLifecycle(e, t) {
		var n = this;
		return n.handleOperation(async () => await rn(n.fetch, n.bucketLifecycleUrl(e), t, { headers: n.headers }));
	}
	...
	bucketLifecycleUrl(e) {
		return `${this.url}/bucket/${Xt(e)}/lifecycle`;
	}
	...
	constructor(e, t = {}, n) {
		let r = e.replace(/\/$/, ""), i = O(O({}, hn), t);
		super(r, i, n, "storage");
	}
	async createBucket(e) {
		var t = this;
		return t.handleOperation(async () => await k(t.fetch, `${t.url}/bucket`, { name: e }, { headers: t.headers }));
	}
	async listBuckets(e) {
		var t = this;
		return t.handleOperation(async () => {
	...
	constructor(e, t = {}, n) {
		let r = e.replace(/\/$/, ""), i = O(O({}, hn), {}, { "Content-Type": "application/json" }, t);
		super(r, i, n, "vectors");
	}
	async createIndex(e) {
		var t = this;
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/CreateIndex`, e, { headers: t.headers }) || {});
	}
	async getIndex(e, t) {
		var n = this;
	...
	async listIndexes(e) {
		var t = this;
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/ListIndexes`, e, { headers: t.headers }));
	}
	...
	async putVectors(e) {
		...
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/PutVectors`, e, { headers: t.headers }) || {});
	}
	async getVectors(e) {
		var t = this;
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/GetVectors`, e, { headers: t.headers }));
	}
	async listVectors(e) {
		var t = this;
		...
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/ListVectors`, e, { headers: t.headers }));
	}
	async queryVectors(e) {
		var t = this;
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/QueryVectors`, e, { headers: t.headers }));
	}
	async deleteVectors(e) {
		var t = this;
		if (e.keys.length < 1 || e.keys.length > 500) throw Error("Keys batch size must be between 1 and 500 items");
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/DeleteVectors`, e, { headers: t.headers }) || {});
	}
}, bn = class extends sn {
	constructor(e, t = {}, n) {
		let r = e.replace(/\/$/, ""), i = O(O({}, hn), {}, { "Content-Type": "application/json" }, t);
	...
	async createBucket(e) {
		var t = this;
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/CreateVectorBucket`, { vectorBucketName: e }, { headers: t.headers }) || {});
	}
	async getBucket(e) {
		var t = this;
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/GetVectorBucket`, { vectorBucketName: e }, { headers: t.headers }));
	}
	async listBuckets(e = {}) {
		var t = this;
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/ListVectorBuckets`, e, { headers: t.headers }));
	}
	async deleteBucket(e) {
		var t = this;
		return t.handleOperation(async () => await A.post(t.fetch, `${t.url}/DeleteVectorBucket`, { vectorBucketName: e }, { headers: t.headers }) || {});
	}
}, xn = class extends bn {
	constructor(e, t = {}) {
	...
	async createIndex(e) {
		...
		return t().call(n, O(O({}, e), {}, { vectorBucketName: n.vectorBucketName }));
	}
	async listIndexes(e = {}) {
...
}, Tn = "2.116.0", j = 3e4, En = 3 * j, Dn = 2 * j, On = "http://localhost:9999", kn = "supabase.auth.token", An = { "X-Client-Info": `gotrue-js/${Tn}` }, jn = "X-Supabase-Api-Version", Mn = { "2024-01-01": {
	timestamp: Date.parse("2024-01-01T00:00:00.0Z"),
	name: "2024-01-01"
} }, Nn = /^([a-z0-9_-]{4})*($|[a-z0-9_-]{3}$|[a-z0-9_-]{2}$)$/i, Pn = "sb_flow_id", Fn = class extends Error {
	constructor(e, t, n) {
...
function qn(e) {
	return M(e) && e.name === "AuthRefreshDiscardedError";
}
...
var z = () => typeof window < "u" && typeof document < "u", B = {
	tested: !1,
	writable: !1
}, cr = () => {
	if (!z()) return !1;
	...
	if (B.tested) return B.writable;
	let e = `lswt-${Math.random()}${Math.random()}`;
	try {
		globalThis.localStorage.setItem(e, e), globalThis.localStorage.removeItem(e), B.tested = !0, B.writable = !0;
	} catch {
		B.tested = !0, B.writable = !1;
	}
	return B.writable;
};
function lr(e) {
...
var ur = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), dr = (e) => typeof e == "object" && !!e && "status" in e && "ok" in e && "json" in e && typeof e.json == "function", V = async (e, t, n) => {
...
function pr(e) {
	...
	for (let e = 0; e < t.length; e++) if (!Nn.test(t[e])) throw new Yn("JWT not in base64url format");
	return {
...
var br = /^[a-zA-Z0-9_-]{8,64}$/;
function xr(e) {
	return typeof e == "string" && br.test(e) ? e : null;
}
function Sr() {
...
var Cr = (e, t) => `${e}-flow-${t}-code-verifier`, wr = (e) => `${e}-flows-code-verifier`;
async function Tr(e, t) {
	let n = await H(e, wr(t));
	return Array.isArray(n) ? n.filter((e) => xr(e) !== null) : [];
}
async function Er(e, t, n, r, i) {
	await V(e, Cr(t, n), r);
	let a = (await Tr(e, t)).filter((e) => e !== n);
	for (a.push(n); a.length > 5;) {
		let n = a.shift();
		await U(e, Cr(t, n)), i?.(n);
	}
	await V(e, wr(t), a), await V(e, `${t}-code-verifier`, r);
}
async function Dr(e, t, n) {
	if (n) {
		let r = await H(e, Cr(t, n));
		return {
			verifier: typeof r == "string" ? r : null,
			flowId: n
		};
	}
	let r = await H(e, `${t}-code-verifier`);
	return {
		verifier: typeof r == "string" ? r : null,
		flowId: null
	};
...
async function W(e, t, n) {
	...
	let i = Cr(t, n), a = await H(e, i);
	await U(e, i);
	let o = await Tr(e, t), s = o.filter((e) => e !== n);
	s.length !== o.length && (s.length > 0 ? await V(e, wr(t), s) : await U(e, wr(t))), a != null && a === await H(e, r) && await U(e, r);
}
async function Or(e, t) {
	let n = await Tr(e, t);
	for (let r of n) await U(e, Cr(t, r));
	await U(e, wr(t)), await U(e, `${t}-code-verifier`);
}
function kr(e, t) {
	...
	let o = r.includes("?") ? "&" : "?";
	return `${r}${o}${Pn}=${encodeURIComponent(t)}${i}`;
}
async function Ar(e, t, n = !1, r) {
	let i = _r(), a = i;
	n && (a += "/recovery");
	let o = Sr();
	await Er(e, t, o, a, r);
...
var jr = /^2[0-9]{3}-(0[1-9]|1[0-2])-(0[1-9]|1[0-9]|2[0-9]|3[0-1])$/i;
function Mr(e) {
	let t = e.headers.get(jn);
	if (!t || !t.match(jr)) return null;
	try {
...
var Fr = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
function G(e) {
	if (!Fr.test(e)) throw Error("@supabase/auth-js: Expected parameter to be UUID but is not");
}
function K(e) {
...
async function Vr(e) {
	if (!dr(e)) throw new Wn(q(e), 0);
	let t;
	try {
		t = await e.json();
	} catch (t) {
		throw Br.includes(e.status) ? new Wn(e.statusText || `HTTP ${e.status}`, e.status) : new N(q(t), t);
	}
	if (Br.includes(e.status)) throw new Wn(q(t), e.status);
	let n, r = Mr(e);
	if (r && r.getTime() >= Mn["2024-01-01"].timestamp && typeof t == "object" && t && typeof t.code == "string" ? n = t.code : typeof t == "object" && t && typeof t.error_code == "string" && (n = t.error_code), !n) {
		if (typeof t == "object" && t && typeof t.weak_password == "object" && t.weak_password && Array.isArray(t.weak_password.reasons) && t.weak_password.reasons.length && t.weak_password.reasons.reduce((e, t) => e && typeof t == "string", !0)) throw new Jn(q(t), e.status, t.weak_password.reasons);
	} else if (n === "weak_password") throw new Jn(q(t), e.status, t.weak_password?.reasons || []);
	else if (n === "session_not_found") throw new F();
	throw new In(q(t), e.status || 500, n);
}
var Hr = (e, t, n, r) => {
	let i = {
		method: e,
...
async function J(e, t, n, r) {
	...
	i["X-Supabase-Api-Version"] || (i[jn] = Mn["2024-01-01"].name), r?.jwt && (i.Authorization = `Bearer ${r.jwt}`);
	let a = r?.query ?? {};
	r?.redirectTo && (a.redirect_to = r.redirectTo);
	let o = await Ur(e, t, n + (Object.keys(a).length ? "?" + new URLSearchParams(a).toString() : ""), {
		headers: i,
...
async function Ur(e, t, n, r, i, a) {
	...
	if (s.ok || await Vr(s), r?.noResolveJson) return s;
	try {
		return await s.json();
	} catch (e) {
		await Vr(e);
...
function Y(e) {
	let t = null;
	Jr(e) && (t = Object.assign({}, e), e.expires_at || (t.expires_at = or(e.expires_in)));
...
var Yr = [
	...
	constructor({ url: e = "", headers: t = {}, fetch: n, experimental: r }) {
		this.url = e, this.headers = t, this.fetch = ur(n), this.experimental = r ?? {}, this.mfa = {
	...
	async signOut(e, t = Yr[0]) {
		if (Yr.indexOf(t) < 0) throw Error(`@supabase/auth-js: Parameter scope must be one of ${Yr.join(", ")}`);
		try {
...
globalThis && cr() && globalThis.localStorage && globalThis.localStorage.getItem("supabase.gotrue-js.locks.debug");
...
function ri(e) {
	...
	let f = ei(e.address), p = `${l ? `${l}://${n}` : n} wants you to sign in with your Ethereum account:\n${f}\n\n${e.statement ? `${e.statement}\n` : ""}`, m = `URI: ${u}\nVersion: ${d}\nChain ID: ${t}${a ? `\nNonce: ${a}` : ""}\nIssued At: ${i.toISOString()}`;
...
function ci(e) {
	...
	let { challenge: t, user: r, excludeCredentials: i } = e, a = n(e, [
		...
		"user",
		"excludeCredentials"
	]), o = L(t).buffer, s = Object.assign(Object.assign({}, r), { id: L(r.id).buffer }), c = Object.assign(Object.assign({}, a), {
		challenge: o,
		user: s
	});
	if (i && i.length > 0) {
...
function li(e) {
	...
	let { challenge: t, allowCredentials: r } = e, i = n(e, ["challenge", "allowCredentials"]), a = L(t).buffer, o = Object.assign(Object.assign({}, i), { challenge: a });
	if (r && r.length > 0) {
...
function ui(e) {
	...
	return {
		...
		rawId: e.id,
		response: {
			attestationObject: R(new Uint8Array(e.response.attestationObject)),
			clientDataJSON: R(new Uint8Array(e.response.clientDataJSON))
		},
		type: "public-key",
...
function di(e) {
	...
	return {
		...
		rawId: e.id,
		response: {
			authenticatorData: R(new Uint8Array(r.authenticatorData)),
			clientDataJSON: R(new Uint8Array(r.clientDataJSON)),
			signature: R(new Uint8Array(r.signature)),
			userHandle: r.userHandle ? R(new Uint8Array(r.userHandle)) : void 0
		},
		type: "public-key",
...
function pi() {
	return !!(z() && "PublicKeyCredential" in window && window.PublicKeyCredential && "credentials" in navigator && typeof (navigator == null ? void 0 : navigator.credentials)?.create == "function" && typeof (navigator == null ? void 0 : navigator.credentials)?.get == "function");
}
async function mi(e) {
	try {
		...
		return t ? t instanceof PublicKeyCredential ? {
			...
			data: null,
			error: new ii("Browser returned unexpected credential type", t)
		} : {
			data: null,
			error: new ii("Empty credential response", t)
		};
	} catch (t) {
		return {
			data: null,
...
var xi = class {
	...
	async _challenge({ factorId: e, webauthn: t, friendlyName: n, signal: r }, i) {
		try {
			...
			let s = r ?? si.createNewAbortSignal();
			...
			switch (a.webauthn.type) {
				case "create": {
					let { data: t, error: n } = await mi({
						publicKey: yi(a.webauthn.credential_options.publicKey, i?.create),
						signal: s
					});
					return t ? {
						data: {
				...
				case "request": {
					let t = bi(a.webauthn.credential_options.publicKey, i?.request), { data: n, error: r } = await hi(Object.assign(Object.assign({}, a.webauthn.credential_options), {
						publicKey: t,
						signal: s
					}));
					return n ? {
			...
			return M(e) ? {
				...
				data: null,
				error: new N("Unexpected error in challenge", e)
			};
		}
	}
	...
	async _authenticate({ factorId: e, webauthn: { rpId: t = typeof window < "u" ? window.location.hostname : void 0, rpOrigins: n = typeof window < "u" ? [window.location.origin] : void 0, signal: r } = {} }, i) {
		...
		try {
			...
			return M(e) ? {
				...
				data: null,
				error: new N("Unexpected error in authenticate", e)
			};
		}
	}
	async _register({ friendlyName: e, webauthn: { rpId: t = typeof window < "u" ? window.location.hostname : void 0, rpOrigins: n = typeof window < "u" ? [window.location.origin] : void 0, signal: r } = {} }, i) {
		...
		try {
			...
			return M(e) ? {
				...
				data: null,
				error: new N("Unexpected error in register", e)
			};
		}
	}
};
...
var Si = {
	...
	get jwks() {
		return Ci[this.storageKey]?.jwks ?? { keys: [] };
	}
	set jwks(e) {
		Ci[this.storageKey] = Object.assign(Object.assign({}, Ci[this.storageKey]), { jwks: e });
	}
	get jwks_cached_at() {
		return Ci[this.storageKey]?.cachedAt ?? -(2 ** 53 - 1);
	}
	set jwks_cached_at(e) {
		Ci[this.storageKey] = Object.assign(Object.assign({}, Ci[this.storageKey]), { cachedAt: e });
	}
	constructor(t) {
		var n;
		...
		let r = Object.assign(Object.assign({}, Si), t);
		if (this.storageKey = r.storageKey, this.instanceID = e.nextInstanceID[this.storageKey] ?? 0, e.nextInstanceID[this.storageKey] = this.instanceID + 1, this.logDebugMessages = !!r.debug, typeof r.debug == "function" && (this.logger = r.debug), this.instanceID > 0 && z()) {
		...
		}, this.persistSession ? (r.storage ? this.storage = r.storage : cr() ? this.storage = globalThis.localStorage : (this.memoryStorage = {}, this.storage = Zr(this.memoryStorage)), r.userStorage && (this.userStorage = r.userStorage)) : (this.memoryStorage = {}, this.storage = Zr(this.memoryStorage)), z() && globalThis.BroadcastChannel && this.persistSession && this.storageKey) {
			try {
	...
	async _initialize() {
		try {
			let e = {}, t = "none";
			if (z() && (e = lr(window.location.href), this._isImplicitGrantCallback(e) ? t = "implicit" : await this._isPKCECallback(e) && (t = "pkce")), z() && this.detectSessionInUrl && t !== "none") {
			...
			return M(e) ? this._returnResult({ error: e }) : this._returnResult({ error: new N("Unexpected error during initialization", e) });
		} finally {
	...
	async signInAnonymously(e) {
		try {
			...
			if (M(e)) return this._returnResult({
				data: {
					user: null,
					session: null
				},
	...
	async signUp(e) {
		...
		try {
			...
			if ("email" in e) {
				...
				this.flowType === "pkce" && ([o, s, t] = await this._getCodeChallengeAndMethod()), n = await J(this.fetch, "POST", `${this.url}/signup`, {
					headers: this.headers,
			...
			if (i || !r) return await W(this.storage, this.storageKey, t), this._returnResult({
				data: {
					user: null,
					session: null
				},
			...
			if (await W(this.storage, this.storageKey, t), M(e)) return this._returnResult({
				data: {
					user: null,
					session: null
				},
	...
	async signInWithEthereum(e) {
		...
		if ("message" in e) t = e.message, n = e.signature;
			...
			let u = ei(l[0]), d = o?.signInWithEthereum?.chainId;
			d ||= ti(await s.request({ method: "eth_chainId" })), t = ri({
				domain: c.host,
				address: u,
				statement: a,
				uri: c.href,
	...
	async _exchangeCodeForSession(e, t) {
		let n = t?.flowId != null, r = n ? xr(t?.flowId) : z() ? xr(lr(window.location.href)[Pn]) : null;
		n && !r && this._debug("#_exchangeCodeForSession()", "provided flowId is not a valid flow id", t?.flowId);
		let { verifier: i, flowId: a } = n && !r ? {
			verifier: null,
			flowId: null
		} : await Dr(this.storage, this.storageKey, r), [o, s] = (i ?? "").split("/");
		try {
			if (!o && this.flowType === "pkce") throw new Un();
			let { data: t, error: n } = await J(this.fetch, "POST", `${this.url}/token?grant_type=pkce`, {
				headers: this.headers,
				body: {
					auth_code: e,
					code_verifier: o
			...
			if (await W(this.storage, this.storageKey, a), n) throw n;
			...
			if (await W(this.storage, this.storageKey, a), M(e)) return this._returnResult({
				data: {
					user: null,
					session: null,
					redirectType: null
	...
	async signInWithOtp(e) {
		...
		try {
			if ("email" in e) {
				...
				let { error: o } = await J(this.fetch, "POST", `${this.url}/otp`, {
					headers: this.headers,
					body: {
						email: n,
						data: r?.data ?? {},
	...
	async signInWithSSO(e) {
		...
		try {
			...
			let i = await J(this.fetch, "POST", `${this.url}/sso`, {
			...
			return i.data?.url && z() && !e.options?.skipBrowserRedirect && window.location.assign(i.data.url), this._returnResult(i);
		} catch (e) {
			if (await W(this.storage, this.storageKey, t), M(e)) return this._returnResult({
				data: null,
				error: e
	...
	async resend(e) {
		...
		try {
			...
			if ("email" in e) {
				...
				let { error: c } = await J(this.fetch, "POST", n, {
					headers: this.headers,
					body: {
						email: r,
						type: i,
				...
				return c && await W(this.storage, this.storageKey, t), this._returnResult({
					data: {
						user: null,
						session: null
					},
			...
			if ("phone" in e) {
				let { phone: t, type: r, options: i } = e, { data: a, error: o } = await J(this.fetch, "POST", n, {
					headers: this.headers,
					body: {
						phone: t,
						type: r,
	...
	async __loadSession() {
		...
		try {
			let e = null, t = await H(this.storage, this.storageKey);
			...
			if (this._debug("#__loadSession()", `session has${n ? "" : " not"} expired`, "expires_at", e.expires_at), !n) {
				if (this.userStorage) {
					let t = await H(this.userStorage, this.storageKey + "-user");
					t?.user ? e.user = t.user : e.user = Lr();
				}
				if (this.storage.isServer && e.user && !e.user.__isUserNotAvailableProxy) {
					let t = { value: this.suppressGetSessionWarning };
					e.user = Rr(e.user, t), t.value && (this.suppressGetSessionWarning = !0);
				}
				return {
					data: { session: e },
					error: null
			...
			if (i) {
				if (e.expires_at && e.expires_at * 1e3 > Date.now()) {
					let t = await H(this.storage, this.storageKey);
	...
	async _getUser(e) {
		try {
			...
			if (M(e)) return Rn(e) && await this._removeSession(), this._returnResult({
				data: { user: null },
				error: e
			});
			throw e;
	...
	async _updateUser(e, t = {}) {
		...
		try {
			return await this._useSession(async (r) => {
				...
				let { data: l, error: u } = await J(this.fetch, "PUT", `${this.url}/user`, {
					headers: this.headers,
			...
			if (await W(this.storage, this.storageKey, n), M(e)) return this._returnResult({
				data: { user: null },
				error: e
			});
			throw e;
	...
	async _setSession(e) {
		try {
			...
			let t = Date.now() / 1e3, n = t, r = !0, i = null, { payload: a } = pr(e.access_token);
	...
	async _getSessionFromURL(e, t) {
		try {
			if (!z()) throw new Bn("No browser detected.");
			...
			switch (t) {
				case "implicit":
					...
					break;
				case "pkce": if (this.flowType === "implicit") throw new Bn("Not a valid implicit grant flow url.");
			}
			if (t === "pkce") {
				if (this._debug("#_initialize()", "begin", "is PKCE flow", !0), !e.code) throw new Hn("No code detected.");
				let { data: t, error: n } = await this._exchangeCodeForSession(e.code, { flowId: e[Pn] });
				if (n) throw n;
				let r = new URL(window.location.href);
				return r.searchParams.delete("code"), r.searchParams.delete(Pn), window.history.replaceState(window.history.state, "", r.toString()), {
					data: {
						session: t.session,
	...
	async _isPKCECallback(e) {
		if (!e.code) return !1;
		let t = xr(e[Pn]);
		return t && await H(this.storage, Cr(this.storageKey, t)) ? !0 : !!await H(this.storage, `${this.storageKey}-code-verifier`);
	}
	...
	async _signOut({ scope: e } = { scope: "global" }) {
		return await this._useSession(async (t) => {
			...
			if (i && !Rn(i)) return this._returnResult({ error: i });
			let a = r.session?.access_token;
			if (a) {
				let { error: t } = await this.admin.signOut(a, e);
				if (t && !(Ln(t) && (t.status === 404 || t.status === 401 || t.status === 403) || Rn(t))) return e !== "others" && await n(), this._returnResult({ error: t });
			}
	...
	async _emitInitialSession(e) {
		return await this._useSession(async (t) => {
			try {
				...
				Rn(t) || Gn(t) || Ln(t) && (t.code === "refresh_token_not_found" || t.code === "refresh_token_already_used" || t.code === "session_expired") ? console.warn(t) : console.error(t);
			}
		});
	}
	async resetPasswordForEmail(e, t = {}) {
		...
		try {
			...
			if (await W(this.storage, this.storageKey, i), M(e)) return this._returnResult({
				data: null,
				error: e
			});
			throw e;
	...
	async linkIdentityOAuth(e) {
		...
		try {
			let { data: n, error: r } = await this._useSession(async (n) => {
				...
				return t = o, await J(this.fetch, "GET", a, {
					headers: this.headers,
					jwt: r.session?.access_token ?? void 0
				});
			});
			if (r) throw r;
			return z() && !e.options?.skipBrowserRedirect && window.location.assign(n?.url), this._returnResult({
				data: {
					provider: e.provider,
					url: n?.url,
					flowId: t
			...
			if (M(n)) return this._returnResult({
				data: {
					provider: e.provider,
					url: null,
					flowId: t
	...
	async linkIdentityIdToken(e) {
		return await this._useSession(async (t) => {
			try {
				...
				if (await W(this.storage, this.storageKey, null), M(e)) return this._returnResult({
					data: {
						user: null,
						session: null
					},
	...
	async _refreshAccessToken(e) {
		...
		try {
			let n = Date.now();
			return await hr(async (n) => (n > 0 && await mr(200 * 2 ** (n - 1)), this._debug(t, "refreshing attempt", n), await J(this.fetch, "POST", `${this.url}/token?grant_type=refresh_token`, {
			...
			if (this._debug(t, "error", e), M(e)) return this._returnResult({
				data: {
					session: null,
					user: null
				},
	...
	async _handleProviderSignIn(e, t) {
		...
		return this._debug("#_handleProviderSignIn()", "provider", e, "options", t, "url", n), z() && !t.skipBrowserRedirect && window.location.assign(n), {
			data: {
				provider: e,
				url: n,
				flowId: r
	...
	async _recoverAndRefresh() {
		...
		try {
			...
			if (t && this.userStorage) {
				let e = await H(this.userStorage, this.storageKey + "-user");
				!this.storage.isServer && Object.is(this.storage, this.userStorage) && !e && (e = { user: t.user }, await V(this.userStorage, this.storageKey + "-user", e)), t.user = e?.user ?? Lr();
			} else if (t && !t.user && !t.user) {
				let e = await H(this.storage, this.storageKey + "-user");
				e && e?.user ? (t.user = e.user, await U(this.storage, this.storageKey + "-user"), await V(this.storage, this.storageKey, t)) : t.user = Lr();
			}
			...
			let n = (t.expires_at ?? Infinity) * 1e3 - Date.now() < En;
			if (this._debug(e, `session has${n ? "" : " not"} expired with margin of ${En}s`), n) {
			...
			this._debug(e, "error", t), Gn(t) ? console.warn(t) : console.error(t);
			return;
		} finally {
			this._debug(e, "end");
		}
	...
	async _callRefreshToken(e) {
		...
		this._debug(r, "begin");
		try {
			this.refreshingDeferred = new fr(), this.refreshingDeferred.promise.then(void 0, () => {});
			let t = await H(this.storage, this.storageKey), { data: n, error: i } = await this._refreshAccessToken(e);
			if (i) throw i;
			if (!n.session) throw new F();
			let a = await H(this.storage, this.storageKey);
			...
			if (await this._saveSession(n.session), this._sessionRemovalEpoch !== o) {
				this._debug(r, "commit guard (post-save): _removeSession ran during _saveSession, undoing write"), await U(this.storage, this.storageKey), this.userStorage && await U(this.userStorage, this.storageKey + "-user");
				let e = {
					data: null,
					error: new Kn()
				};
			...
			if (this._debug(r, "error", i), M(i)) {
				let n = {
					data: null,
					error: i
				};
				if (!Gn(i)) {
					let e = await H(this.storage, this.storageKey);
	...
	async _saveSession(e) {
		...
		if (this.userStorage) {
			!n && t.user && await V(this.userStorage, this.storageKey + "-user", { user: t.user });
			let e = Object.assign({}, t);
			delete e.user;
			let r = zr(e);
			await V(this.storage, this.storageKey, r);
		} else {
			let e = zr(t);
			await V(this.storage, this.storageKey, e);
		}
	}
	async _removeSession() {
		this._sessionRemovalEpoch += 1, this._debug("#_removeSession()"), this.lastRefreshFailure = null, this.suppressGetSessionWarning = !1, await U(this.storage, this.storageKey), await Or(this.storage, this.storageKey), await U(this.storage, this.storageKey + "-user"), this.userStorage && await U(this.userStorage, this.storageKey + "-user"), await this._notifyAllSubscribers("SIGNED_OUT", null);
	}
	...
	async _startAutoRefresh() {
		...
		let e = setInterval(() => this._autoRefreshTokenTick(), j);
	...
	async _autoRefreshTokenTick() {
		if (this._debug("#_autoRefreshTokenTick()", "begin"), this.lock != null) {
			try {
					...
					try {
						...
						try {
							return await this._useSession(async (t) => {
								...
								let r = Math.floor((n.expires_at * 1e3 - e) / j);
								this._debug("#_autoRefreshTokenTick()", `access token expires in ${r} ticks, a tick lasts ${j}ms, refresh threshold is 3 ticks`), r <= 3 && await this._callRefreshToken(n.refresh_token);
							});
						} catch (e) {
				...
				if (e instanceof Qr) this._debug("auto refresh token tick lock not available");
				else throw e;
			}
			return;
		}
	...
	async _handleVisibilityChange() {
		if (this._debug("#_handleVisibilityChange()"), !z() || !(window != null && window.addEventListener)) return this.autoRefreshToken && this.startAutoRefresh(), !1;
		try {
	...
	_maybeAppendFlowIdToRedirect(e, t) {
		return !e || !t || !this.experimental.appendPkceFlowIdToRedirects ? e ?? void 0 : kr(e, t);
	}
	async _getCodeChallengeAndMethod(e = !1) {
		return Ar(this.storage, this.storageKey, e, (e) => this._debug("#_getCodeChallengeAndMethod()", "evicted oldest pending PKCE verifier slot", e));
	}
	async _unenroll(e) {
		try {
			return await this._useSession(async (t) => {
			...
			try {
				return await this._useSession(async (t) => {
					...
					switch (a.webauthn.type) {
						...
						case "request": return {
							data: Object.assign(Object.assign({}, a), { webauthn: Object.assign(Object.assign({}, a.webauthn), { credential_options: Object.assign(Object.assign({}, a.webauthn.credential_options), { publicKey: li(a.webauthn.credential_options.publicKey) }) }) }),
							error: null
						};
					}
				});
	...
	async _getAuthenticatorAssuranceLevel(e) {
		...
		let { payload: r } = pr(t.access_token), i = null;
		r.aal && (i = r.aal);
		let a = i;
	...
	async _getRecoveryCodesStatus() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await J(this.fetch, "GET", `${this.url}/factors/recovery-codes`, {
					headers: this.headers,
	...
	async _generateRecoveryCodes(e) {
		...
		try {
			return await this._useSession(async (t) => {
				...
				let { data: i, error: a } = await J(this.fetch, "POST", `${this.url}/factors/recovery-codes`, {
		...
		let t = async () => {
			try {
				return await this._useSession(async (t) => {
					...
					let { data: i, error: a } = await J(this.fetch, "POST", `${this.url}/factors/recovery-codes/verify`, {
						body: { code: e.code },
						headers: this.headers,
						jwt: n?.session?.access_token
					});
					...
					let o = Object.assign({ expires_at: or(i.expires_in) }, i);
	...
	async _regenerateRecoveryCodes() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await J(this.fetch, "POST", `${this.url}/factors/recovery-codes/regenerate`, {
					headers: this.headers,
					jwt: t?.session?.access_token
				});
				return i ? this._returnResult({
	...
	async _unenrollRecoveryCodes() {
		...
		try {
			return await this._useSession(async (e) => {
				...
				let { data: r, error: i } = await J(this.fetch, "DELETE", `${this.url}/factors/recovery-codes`, {
					headers: this.headers,
					jwt: t?.session?.access_token
				});
				return i ? this._returnResult({
	...
	async _approveAuthorization(e, t) {
		try {
			return await this._useSession(async (n) => {
				...
				return a.data && a.data.redirect_url && z() && !t?.skipBrowserRedirect && window.location.assign(a.data.redirect_url), a;
			});
		} catch (e) {
			if (M(e)) return this._returnResult({
				data: null,
	...
	async getClaims(e, t = {}) {
		try {
			...
			let { header: r, payload: i, signature: a, raw: { header: o, payload: s } } = pr(n);
			...
			let l = Pr(r.alg), u = await crypto.subtle.importKey("jwk", c, l, !0, ["verify"]);
			if (!await crypto.subtle.verify(l, u, a, ar(`${o}.${s}`))) throw new Yn("Invalid JWT signature");
			return {
				data: {
					claims: i,
					header: r,
	...
	async signInWithPasskey(e) {
		K(this.experimental);
		try {
			if (!pi()) return this._returnResult({
				data: null,
				error: new N("Browser does not support WebAuthn", null)
			});
			...
			let { data: r, error: i } = await hi({
				publicKey: li(t.options),
				signal: e?.options?.signal ?? si.createNewAbortSignal()
			});
			if (i || !r) return this._returnResult({
				data: null,
				error: i ?? new N("WebAuthn ceremony failed", null)
			});
			let a = di(r);
...
var Ei = Ti, Di = "2.116.0", Oi = "", ki;
if (typeof Deno < "u") Oi = "deno", ki = Deno.version?.deno;
else if (typeof document < "u") Oi = "web";
else if (typeof navigator < "u" && navigator.product === "ReactNative") Oi = "react-native";
else {
	var Ai;
	Oi = "node";
	let e = globalThis.process;
	ki = e == null || (Ai = e.version) == null ? void 0 : Ai.replace(/^v/, "");
}
var ji = [`runtime=${Oi}`];
ki && ji.push(`runtime-version=${ki}`);
var Mi = { headers: { "X-Client-Info": `supabase-js/${Di}; ${ji.join("; ")}` } }, Ni = { schema: "public" }, Pi = {
	autoRefreshToken: !0,
	persistSession: !0,
	detectSessionInUrl: !0,
	flowType: "implicit"
...
var Ki = (e) => e ? (...t) => e(...t) : (...e) => fetch(...e), qi = () => Headers, Ji = (e) => e.startsWith("sb_publishable_") || e.startsWith("sb_secret_"), Yi = "sb_temp_", Xi = /* @__PURE__ */ new Set(), Zi = (e) => {
	if (!e.startsWith("sb_") || Ji(e) || e.startsWith(Yi)) return;
	let t = e.match(/^sb_[a-zA-Z0-9]+_/)?.[0] ?? "unknown";
	Xi.has(t) || (Xi.add(t), console.warn("@supabase/supabase-js: Unrecognized Supabase API key format. The client will proceed and send this key as-is; if you see authentication errors you may need to upgrade @supabase/supabase-js to a version that recognizes this key type."));
}, Qi = (e, t, n, r, i, a) => {
	let o = Ki(r), s = qi(), c = i?.enabled === !0, l = i?.respectSamplingDecision !== !1, u = c ? Bi(t) : null, d = !(a?.omitApiKeyAsBearer && Ji(e));
	return async (t, r) => {
...
var ia = !1;
function aa(e) {
	ia || typeof e == "object" && e && "schema" in e && e.schema !== void 0 && (ia = !0, console.warn("@supabase/supabase-js: The \"schema\" option must be nested under \"db\", e.g. createClient(url, key, { db: { schema: 'myschema' } }). A top-level \"schema\" is ignored and queries go to the default schema."));
}
function oa(e, t) {
...
var ca = class extends Ei {
	...
	constructor(e, t, n) {
		...
		Zi(t), aa(n), this.realtimeUrl = new URL("realtime/v1", r), this.realtimeUrl.protocol = this.realtimeUrl.protocol.replace("http", "ws"), this.authUrl = new URL("auth/v1", r), this.storageUrl = new URL("storage/v1", r), this.functionsUrl = new URL("functions/v1", r);
		let i = `sb-${r.hostname.split(".")[0]}-auth-token`, a = {
			db: Ni,
			realtime: Fi,
			auth: Q(Q({}, Pi), {}, { storageKey: i }),
			global: Mi,
			tracePropagation: Ii
		}, o = oa(n ?? {}, a);
		...
		} })) : this.auth = this._initSupabaseAuthClient(o.auth ?? {}, this.headers, o.global.fetch), this.fetch = Qi(t, e, this._getSessionToken.bind(this), o.global.fetch, o.tracePropagation), this.functionsFetch = Qi(t, e, this._getSessionToken.bind(this), o.global.fetch, o.tracePropagation, { omitApiKeyAsBearer: !0 }), this.realtime = this._initRealtimeClient(Q({
			headers: this.headers,
	...
	_initRealtimeClient(e) {
		return new Et(this.realtimeUrl.href, Q(Q({}, e), {}, { params: Q(Q({}, { apikey: this.supabaseKey }), e?.params) }));
	}
...
da() && console.warn("⚠️  Node.js 20 and below are deprecated and will no longer be supported in future versions of @supabase/supabase-js. Please upgrade to Node.js 22 or later. For more information, visit: https://github.com/orgs/supabase/discussions/45715");
var $ = ua("https://kfdmduogwpgolhybqeez.supabase.co", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtmZG1kdW9nd3Bnb2xoeWJxZWV6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODk0Nzg4OTQsImV4cCI6MjEwNTA1NDg5NH0.lnT7Fiii9mABkyze2OnLIZ9LMPojgkxv9YrUZjQ4Mnc", { auth: {
	storage: {
		getItem: (e) => chrome.storage.local.get(e).then((t) => t[e] ?? null),
		setItem: (e, t) => chrome.storage.local.set({ [e]: t }),
		removeItem: (e) => chrome.storage.local.remove(e)
	},
	persistSession: !0,
	autoRefreshToken: !0,
...
//#endregion
//#region extension/background.js
chrome.runtime.onMessage.addListener((e, t, n) => (fa(e, t).then(n).catch((e) => {
	console.error("[background] Error handling message:", e), n({
		success: !1,
		error: e.message
	});
}), !0));
async function fa(e, t) {
	switch (e.action) {
		case "get-session": return ha();
		case "sign-in": return ga(e.email, e.password);
		case "sign-up": return _a(e.email, e.password);
		case "sign-out": return va();
		case "save-bookmark": return ya();
		case "page-content": return ba(e, t);
		case "explain-keyword": return pa(e.keyword, e.context);
		case "save-flashcard": return ma(e);
		default: return {
			success: !1,
...
chrome.commands.onCommand.addListener(async (e) => {
	if (e !== "explain-selection") return;
	let [t] = await chrome.tabs.query({
		active: !0,
		currentWindow: !0
	});
	if (t?.id && t.url && !t.url.startsWith("chrome://") && !t.url.startsWith("chrome-extension://")) try {
		await chrome.scripting.executeScript({
			target: { tabId: t.id },
			files: ["content-script.js"]
		}), await chrome.tabs.sendMessage(t.id, { action: "show-explain-popup" });
	} catch (e) {
		console.error("[background] Failed to trigger explain popup:", e);
	}
});
async function pa(e, t) {
	let { data: n, error: r } = await $.functions.invoke("explain-keyword", { body: {
		keyword: e,
		context: t
	} });
	return r || !n?.success ? {
		success: !1,
		error: r?.message || n?.error || "Không thể giải thích từ khóa"
	} : {
		success: !0,
		data: {
...
async function ma({ keyword: e, explanation: t, source_url: n }) {
	let { data: { session: r }, error: i } = await $.auth.getSession();
	if (i || !r) return {
		success: !1,
		error: "Not authenticated"
	};
	let { error: a } = await $.from("flashcards").insert({
		user_id: r.user.id,
		keyword: e,
		explanation: t,
		source_url: n
...
async function ha() {
	let { data: { session: e }, error: t } = await $.auth.getSession();
	return t ? {
		success: !1,
		error: t.message
	} : {
...
async function ga(e, t) {
	let { data: n, error: r } = await $.auth.signInWithPassword({
		email: e,
		password: t
	});
	return r ? {
...
async function _a(e, t) {
	let { data: n, error: r } = await $.auth.signUp({
		email: e,
		password: t
	});
	return r ? {
...
async function va() {
	let { error: e } = await $.auth.signOut();
	return e ? {
		success: !1,
		error: e.message
	} : { success: !0 };
...
async function ya() {
	...
	if (t || !e) return {
		success: !1,
		error: "Sign in before saving a page."
	};
	...
	if (!n.url || !/^https?:\/\//.test(n.url)) return {
		success: !1,
	...
	try {
		return await chrome.scripting.executeScript({
			target: { tabId: n.id },
			func: () => {
				window.__growCareerExtractMode = !0;
			}
		}), await chrome.scripting.executeScript({
			target: { tabId: n.id },
			files: ["content-script.js"]
		...
		return console.error("[background] Failed to inject content-script:", e), {
			success: !1,
			error: "Failed to read page content: " + e.message
		};
	}
}
async function ba(e, t) {
	let { title: n, text: r } = e, i = t.tab;
	if (!i) return console.error("[background] handlePageContent: no sender tab"), {
		success: !1,
		error: "No sender tab"
	};
	let { data: { session: a }, error: o } = await $.auth.getSession();
	if (o || !a) return console.error("[background] No session when saving bookmark"), chrome.runtime.sendMessage({
		action: "bookmark-result",
		success: !1,
		error: "Not authenticated"
	}).catch(() => {}), {
		success: !1,
	...
	let s = i.url, c = n || i.title || s, l = r || "", u = a.user.id;
	console.log(`[background] Inserting bookmark: ${c} | ${s}`);
	let { data: d, error: f } = await $.from("bookmarks").insert({
		user_id: u,
		url: s,
		title: c,
		raw_text: l.slice(0, 5e4),
		status: "pending"
	}).select().single();
	return f ? (console.error("[background] Insert error:", f), chrome.runtime.sendMessage({
		action: "bookmark-result",
		success: !1,
		error: f.message
	}).catch(() => {}), {
		success: !1,
		error: f.message
	}) : (console.log("[background] Bookmark saved, id:", d.id, "— AI pipeline will run automatically"), chrome.runtime.sendMessage({
		action: "bookmark-result",
		success: !0,
		bookmark: d
	}).catch(() => {}), {
