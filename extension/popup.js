import { getWebsiteUrl } from './website.js';
// popup.js
...
// Store the website address separately from the authenticated session.
document.getElementById('btn-workspace').addEventListener('click', async () => {
  try {
    const websiteUrl = await getWebsiteUrl();
    await chrome.tabs.create({ url: new URL('/learn#highlights', websiteUrl).href });
  } catch {
...
document.getElementById('btn-privacy').addEventListener('click', async () => {
  try { await chrome.tabs.create({ url: new URL('/privacy', await getWebsiteUrl()).href }); }
