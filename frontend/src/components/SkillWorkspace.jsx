...
export function SkillWorkspace() {
  ...
  return <div className="sw-app">
    ...
    <div className="sw-body"><div className="sw-topbar"><span>Workspace <span>/</span> <strong>{navigation.find(n=>n[0]===view)?.[2]||'Reading'}</strong></span><span className="sw-demo"><i/> Interactive demo</span></div><main key={view} className="sw-main" tabIndex={-1}>{!['map','roadmap','highlights','flashcards'].includes(view)&&<p className="sw-demo-note">Sample learning data · Changes last for this session · No AI service connected</p>}{notice&&<div className="sw-notice" role="status">✓ {notice}<button aria-label="Dismiss notification" onClick={()=>setNotice('')}>×</button></div>}
