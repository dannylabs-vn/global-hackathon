...
import { analyzeCareer, getCareerReports, getBookmarks, deleteBookmark, getAnalyticsData, getFlashcards, generatePracticeQuestion } from '../lib/api.js';
...
export function SkillWorkspace() {
  ...
  const [practiceAnsweredCount,setPracticeAnsweredCount] = useState(0);
  ...
  async function loadPracticeQuestion() {
    ...
    const { data, error } = await generatePracticeQuestion();
    setPracticeLoading(false);
    if (error || !data?.success) { setPracticeError(error?.message||'Could not generate a practice question.'); setPracticeQuestion(null); return; }
  ...
  useEffect(()=>{ if (view==='practice' && !practiceLoaded) { setPracticeLoaded(true); loadPracticeQuestion(); } },[view,practiceLoaded]);
  ...
  return <div className="sw-app">
    ...
    {view==='practice'&&<>{heading('TRY. REFLECT. GROW.','Put curiosity into practice','A quick multiple-choice question generated from a keyword you saved.')}
    {practiceError&&<div className="sw-notice sw-notice-error" role="alert">⚠ {practiceError}<button aria-label="Dismiss error" onClick={()=>setPracticeError('')}>×</button></div>}
    <div className="sw-two-col"><section className="sw-panel sw-practice">
    {practiceLoading?<p className="sw-small">Generating a question from your flashcards…</p>
    :!practiceQuestion?<div className="sw-empty"><h2>No practice question yet</h2><p>Highlight a keyword on any page with the extension (Ctrl+Shift+E) and save it as a flashcard — Practice generates questions from what you save.</p>{button('Try again',loadPracticeQuestion)}</div>
    :<><div className="sw-panel-title"><span className="sw-eyebrow">{practiceQuestion.keyword.toUpperCase()}</span><span className="sw-tag">≈ 1 min</span></div><p className="sw-eyebrow">THE QUESTION</p><h2>{practiceQuestion.question}</h2><fieldset disabled={practiceSubmitted}><legend className="sw-sr-only">Choose your answer</legend>{practiceQuestion.options.map((option,i)=><label className={`sw-option ${practiceAnswer===option?'selected':''} ${practiceSubmitted&&option===practiceQuestion.correct_answer?'correct':''}`} key={option}><input type="radio" name="answer" checked={practiceAnswer===option} onChange={()=>setPracticeAnswer(option)}/><span className="sw-letter">{'ABC'[i]}</span><span>{option}</span>{practiceSubmitted&&option===practiceQuestion.correct_answer&&<b>✓</b>}</label>)}</fieldset>{!practiceSubmitted?<button className="sw-button" disabled={practiceAnswer===null} onClick={()=>{setPracticeSubmitted(true);setPracticeAnsweredCount(c=>c+1);}}>Check my answer ↗</button>:<div className="sw-feedback" role="status"><p className="sw-eyebrow">A MOMENT TO REFLECT</p><h3>{practiceAnswer===practiceQuestion.correct_answer?'You found a useful starting point.':'Another way to look at it.'}</h3><p>{practiceQuestion.explanation}</p><div className="sw-actions">{button(plan.includes(practiceQuestion.keyword)?'Added to roadmap ✓':'Add to roadmap',()=>addToPlan(practiceQuestion.keyword),true)}{button('Next question →',loadPracticeQuestion)}</div></div>}</>}
    </section><aside><section className="sw-focus"><p className="sw-eyebrow">FOCUS MODE</p><h2>Understanding<br/>starts with trying.</h2><p>There’s no score to chase. Notice how you approach the problem, then take one idea into your next attempt.</p></section><section className="sw-panel sw-up-next"><p className="sw-eyebrow">THIS SESSION</p><p className="sw-small">You've answered <strong>{practiceAnsweredCount}</strong> {practiceAnsweredCount===1?'question':'questions'} so far.</p>{practiceQuestion&&!practiceLoading&&button('Skip to another question ↗',loadPracticeQuestion,true)}</section></aside></div></>}
