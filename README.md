...
The main **Practice** page uses Duy's `03b3286` quiz flow: a question from a saved keyword, three answer options, feedback/explanation, Next/Skip controls, and the answered count for the current workspace session. The new `/api/practice-question` Vercel endpoint replaces the Gemini-backed Supabase `generate-practice-question` invocation. It verifies the signed-in user, reads up to 1,000 recent flashcards using that user's JWT and RLS, selects one usable card randomly, and asks DeepSeek for the question. It rejects missing flashcards, duplicate options, or a correct answer outside the options. No new database schema or service-role key is needed. Practice and roadmap generation share `server/deepseek.js` and the same `DEEPSEEK_API_KEY` / optional `DEEPSEEK_MODEL` configuration. The Gemini Edge Function is left untouched and is no longer called by Practice.

Roadmap exercises remain available from **Career Analysis → your roadmap → Open session** (`/learn#session`). Their completion is self-reported after answering the question and recording work/reflection. Plans, roadmap answers, notes, and completion are stored per account in this browser; **they do not sync across devices**. Export your plan to keep a copy. Generating a replacement plan resets session progress only after successful generation. Practice's **Add to roadmap** carries the saved keyword into the planner's constraints; the user confirms the career and generates an updated plan before the roadmap changes.
